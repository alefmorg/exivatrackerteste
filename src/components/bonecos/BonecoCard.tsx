      ...
      {settings.showCredentials && (
        <div className="space-y-1.5 text-sm bg-secondary/50 rounded-lg p-3 mb-3">
          ...
          <div className="flex items-center gap-2 text-muted-foreground">
            ...
            {b.email && <button onClick={() => onCopy(b.email)} className="text-muted-foreground hover:text-primary"><Copy className="h-3.5 w-3.5" /></button>}
          </div>
          <div className="flex items-center gap-2 text-muted-foreground">
            ...
            <button onClick={() => onTogglePassword(b.id)} className="text-muted-foreground hover:text-primary">
              {visiblePasswords.has(b.id) ? <EyeOff className="h-3.5 w-3.5" /> : <Eye className="h-3.5 w-3.5" />}
            </button>
            <button onClick={() => onCopy(b.password)} className="text-muted-foreground hover:text-primary"><Copy className="h-3.5 w-3.5" /></button>
          </div>
          <div className="flex items-center gap-2 text-muted-foreground">
            ...
            <button onClick={() => onToggleToken(b.id)} className="text-muted-foreground hover:text-primary">
      ...
      <div className="flex items-center justify-between pt-3 border-t border-border text-xs text-muted-foreground">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => onSync(b)} disabled={syncing}
            className="text-muted-foreground hover:text-primary transition-colors disabled:opacity-50" title="Sincronizar com TibiaData">
            <RefreshCw className={`h-3.5 w-3.5 ${syncing ? 'animate-spin text-primary' : ''}`} />
          </button>
          <Button variant={b.used_by ? 'outline' : 'default'} size="sm"
          ...
          {isAdmin && (
            <>
              <button onClick={() => onEdit(b)} className="text-primary hover:underline font-medium">Editar</button>
              <button onClick={() => onDelete(b.id, b.name)} className="text-offline hover:underline">Excluir</button>
            </>
          )}
        </div>
      </div>
